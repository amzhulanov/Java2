...
public class MultiThreading {
    ...
    private static float[] arrMultiThread2 = new float[size];
    ...
    public static long speedMultiThreading() throws InterruptedException {
        ...
        System.arraycopy(arrOneThread, h, arrMultiThread2, h - 1, h);
        long a = System.currentTimeMillis();
        FirstThread();
        SecondThread();
        ...
        System.arraycopy(arrMultiThread2, h - 1, arrOneThread, h, h);
    ...
    private static void SecondThread() {
        new Thread() {
            ...
            public void run() {
                for (int n = 0;
                     n < h - 1; n++) {
                    arrMultiThread2[n] = (float) (arrMultiThread2[n] * Math.sin(0.2f + n/ 5) * Math.cos(0.2f + n / 5) * Math.cos(0.4f + n / 2));
                }
                waitThread2 = true;
            }
        }.start();
